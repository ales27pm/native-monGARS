...
# Load environment variables from special env file
if [ -f specialenv/.env ]; then
    source specialenv/.env
    echo "✅ Loaded credentials from specialenv/.env"
elif [ -f .env ]; then
    source .env
    echo "✅ Loaded credentials from .env"
else
    echo "❌ Neither specialenv/.env nor .env file found!"
    echo "Please create specialenv/.env with your GitHub credentials"
    exit 1
fi
