...
export class AutonomousSystem {
  ...
  private async setupBackgroundTasks(): Promise<void> {
    try {
      ...
      // Note: Background task registration would typically be done
      // through expo-background-fetch or similar, but since it's deprecated,
      // we'll rely on TaskManager.defineTask for now
      console.log('ARIA: Background task defined successfully');
